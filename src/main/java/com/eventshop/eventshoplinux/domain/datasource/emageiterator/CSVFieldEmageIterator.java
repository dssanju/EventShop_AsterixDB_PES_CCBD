...
public class CSVFieldEmageIterator extends EmageIterator
{
	...
	int latIndex = -1, lngIndex = -1, valIndex = -1;	// no specific column number provided
	...
	public CSVFieldEmageIterator (FrameParameters fp, String th, String url, String ds, String sw){
		params = fp;
		this.theme =th;
		this.fileURL = url;
		...
		queue = new LinkedBlockingQueue<Emage>();
		isRunning = true;
		setResolutionWrapper(DatasourceType.valueOf(ds), SpatialWrapper.valueOf(sw));
	}
	
	public CSVFieldEmageIterator (FrameParameters fp, String th, String url, DatasourceType ds, SpatialWrapper sw){
		params = fp;
		this.theme =th;
		this.fileURL = url;
	...
	// Select particular column index for lat, long, and/or value fields (start from 0)
	// latitude and longitude fields are required, value field is optional
	// if value's index is not provide (set = -1), the default value of the value is 1.0
	...
	public void run() 
	{
		...
		curWindowEnd = (long)Math.ceil(System.currentTimeMillis() / params.timeWindow) * params.timeWindow  + params.syncAtMilSec;
		...
		while(isRunning)
		{
			...
			try {
				...
				log.info("CSVFieldEmageIterator (" + e.theme + ") process time: " + (System.currentTimeMillis() - startEit) + " ms");
				...
				while(now < curWindowEnd)
				{
					...
					try {
						...
						log.info("woke up");
						now = System.currentTimeMillis();
						myPoints = new ArrayList<DataPoint>();	//  we don't accumulate datapoints from previous time window
	...
	public Emage getTheEmage(long curWindowEnd)
	{
		...
		try {
			if(this.latIndex == -1)
				...
				readPointsFromCSVFile(this.latIndex, this.lngIndex, this.valIndex);
		...
		log.info("CSVFieldEmageIterator readpointsfromfile process time: " + (System.currentTimeMillis() - startGetEmage) + "ms");
		/*
		for(int p = 0; p < myPoints.size(); p++){
			Point aPoint = latLong2Pixel(myPoints.get(p).lat, myPoints.get(p).lng);
			//dataGrid[aPoint.x][aPoint.y] += myPoints.get(p).value;
			for(int i = 0; i < nRows; i++){
				for(int j = 0; j < nCols; j++){
					Point bPoint = new Point(i, j);
					dataGrid[i][j] += myPoints.get(p).value /(Math.pow(2,0.1*bPoint.distance(aPoint)));
				}
			}
		}
		*/
		...
		*/
		...
		e1.image = dataGrid;
		//log.info("test emage is generated or not? " + e1.toJson().toString());
		return e1 ;
	}

	/*
	 *  before having ResolutionWrapper
	...
	private double gridValue(int i, int j)
	{
		...
		for (int s=0; s< myPoints.size(); s++)
		{
			Point aPoint = new Point(myPoints.get(s).xEmageIndex, myPoints.get(s).yEmageIndex);
	...
	public PlanarImage gridToEmage(double [][] dataGrid, String imageName) throws IOException
	{
		...
		try {
			System.out.println(Config.getProperty(TEMPDIR) + imageName + ".jpg");
			ImageIO.write(rectImage, "jpg", new File(Config.getProperty(TEMPDIR) + imageName+".jpg"));
	...
	// Provide column index of lat, long, and value field inside the CSV file (start with 0)
	// The latitude and longitude fields are required, but the value field is optional.
	// If value field is not provide (valueIndex == -1), the default value is 1.0.
	public void readPointsFromCSVFile(int latIndex, int lngIndex, int valueIndex) throws IOException{
		URL url = new URL(fileURL);
		
		InputStream stream = url.openStream();
		
		BufferedReader reader = null;
		String csvdelimiter = Config.getProperty("csvdelimiter");
		...
		if (dataRow != null)
			dataRow = reader.readLine();		// ignore the first line. It is the header
		while(dataRow != null){
			String[] dataArray = dataRow.split(csvdelimiter);
			if(dataArray.length >= 2){
				...
				if(valueIndex != -1){
					myPoints.add(new DataPoint(lat, lng, Double.parseDouble(dataArray[valueIndex]), p));
					//log.info("lat,long,value:" + lat +"," + lng + "," + dataArray[valueIndex] );
				} else {
	...
	public void readPointsFromCSVFile() throws IOException
	{
		...
		while(dataRow != null){
			...
			if(dataArray.length == 3){
				...
				//log.info("valid format:" + lat + "," + lng + "," + val + "," + p.x + "," + p.y);
			} else {
	...
	public Point latLong2Pixel(Double latV, Double longV)
	{
		...
		int x=(nRows-1)-(int)((BigDecimal.valueOf(latV)).subtract(BigDecimal.valueOf(params.swLat), context).divide(BigDecimal.valueOf(params.latUnit), context).doubleValue());
		int y=(int)((BigDecimal.valueOf(longV)).subtract(BigDecimal.valueOf(params.swLong), context).divide(BigDecimal.valueOf(params.longUnit), context).doubleValue());
	...
	public static void main(String[] args) {
		try {
			long timeWindow = 1000*60; //1mins for testing//*60*24*7; // the last 7 days
			long sync = 1000; 
			double latUnit = 2;
			double longUnit = 2;
			double swLat = 20;
			...
			//String tempFile = "https://docs.google.com/spreadsheet/pub?key=0Auv1FPgY5UScdHNia09KM2o4c0N5aThqMGRXYU5jbEE&output=html";
			//String tempFile = "https://www.dropbox.com/s/qp1yh7be112wpkn/csvfield.csv";
			String tempFile = "http://eventshop.ics.uci.edu:8004/sln/datasource/csvfield";
			//Emage e1 = new Emage(fp, theme);
			CSVFieldEmageIterator csvEit = new CSVFieldEmageIterator(fp, theme, tempFile, DatasourceType.point,SpatialWrapper.count);
			csvEit.run();
		} catch (Exception e){
			log.error(e.getMessage());
		}
