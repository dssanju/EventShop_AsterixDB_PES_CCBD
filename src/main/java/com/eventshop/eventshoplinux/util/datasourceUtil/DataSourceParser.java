	...
	public DataProcess processData(DataSource src)
	{
		...
		{			
			try {
				...
				System.out.println("----visual param trans matrix noRows: "+ src.visualParam.translationMatrix.getRow() 
						+ ", colorMat noRows: " + src.visualParam.colorMatrix.getRow());
				...
				VisualEmageIterator veit = new VisualEmageIterator(src.initParam, src.srcTheme, src.url, src.visualParam.translationMatrix.getMatrix(), 
						src.visualParam.colorMatrix.getMatrix(), src.visualParam.maskPath, src.visualParam.ignoreSinceNumber);
		...
		{			
			...
			{				
				...
				log.info("wrapper created");
				process = new DataProcess(merger, csvEIter, null, filepath, imgBasePath+src.srcID, src.srcID);
			...
			else if(src.supportedWrapper.equalsIgnoreCase("csvField"))
			{
				...
				log.info(src.initParam.numOfColumns + ", " + src.initParam.numOfRows + " after cal: " + src.initParam.getNumOfColumns() + ", " + src.initParam.getNumOfRows());
				CSVFieldEmageIterator csvEIter = new CSVFieldEmageIterator(src.initParam, src.srcTheme, src.url, keyValue.get("datasource_type").getAsString(), keyValue.get("spatial_wrapper").getAsString());
				csvEIter.setLatLongValIndex(keyValue.get("lat_index").getAsInt(), keyValue.get("lon_index").getAsInt(), keyValue.get("val_index").getAsInt());
				csvEIter.setSrcID(Long.parseLong(src.srcID));
				log.info(src.srcTheme+", "+ src.url+", "+ keyValue.get("datasource_type").getAsString()+", "+ keyValue.get("spatial_wrapper").getAsString()
						+ ", "+ keyValue.get("lat_index").getAsInt()+", "+ keyValue.get("lon_index").getAsInt()+", "+ keyValue.get("val_index").getAsInt());
				log.info("wrapper created");
				process = new DataProcess(merger, csvEIter, null, filepath, imgBasePath+src.srcID, src.srcID);
